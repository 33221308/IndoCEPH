...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        show_angle = float(hyp.get('angle_lambda', 0) or 0) > 0
        cols = ['Epoch','GPU_mem','box_loss','cls_loss','dfl_loss']
        if float(hyp.get('angle_lambda', 0) or 0) > 0:
            cols.append('angle')
        cols += ['Instances','Size']
        LOGGER.info(('\n' + '%11s' * len(cols)) % tuple(cols))


        # show_angle = hyp.get('angle_lambda', 0) and float(hyp.get('angle_lambda', 0)) > 0
        # cols = ['Epoch', 'GPU_mem', 'box_loss', 'cls_loss', 'dfl_loss']
        # if show_angle:
        #     cols.append('angle')  # kolom ekstra bila angle aktif
        # cols += ['Instances', 'Size']
        # LOGGER.info(('\n' + '%11s' * len(cols)) % tuple(cols))
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # --- init/update mloss agar match panjang loss_items (3 atau 4) ---
            if (mloss is None) or (mloss.numel() != loss_items.numel()):
                mloss = torch.zeros_like(loss_items, device=loss_items.device)

            # DDP / quad scaling (di luar autocast tidak masalah)
            if RANK != -1:
                loss *= WORLD_SIZE
            if opt.quad:
                loss *= 4.
            ...
            if RANK in {-1, 0}:
                ...
                # progress bar (format dinamis)
                fmt = '%11s%11s' + ('%11.4g' * mloss.numel()) + '%11.4g%11.4g'
                vals = [f'{epoch}/{epochs - 1}',
                        f'{torch.cuda.memory_reserved() / 1E9 if torch.cuda.is_available() else 0:.3g}G',
                    ] + [x.item() for x in mloss] + [float(targets.shape[0]), float(imgs.shape[-1])]
                pbar.set_description(fmt % tuple(vals))
